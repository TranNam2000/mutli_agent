...
- Progressive enhancement — form work không cần JS
...
Use `Read` + `Edit` / `Write` on the real project files. Run `flutter analyze` (or equivalent) via `Bash` before finishing. Output only a summary of what you changed — no source blocks.
